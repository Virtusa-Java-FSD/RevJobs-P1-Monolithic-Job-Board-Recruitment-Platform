...
const AdminDashboard: React.FC = () => {
  ...
  const fetchUsers = async () => {
    try {
      ...
      if (currentUser.email && !localUsers.find(u => u.email === currentUser.email)) {
        localUsers.push({
      ...
      const allUsers = [...backendUsers, ...localUsers];
      ...
      const jobSeekerLogins = usersWithStats.filter(u => u.role === 'JOB_SEEKER').reduce((sum, u) => sum + u.loginCount, 0);
      const employerLogins = usersWithStats.filter(u => u.role === 'EMPLOYER').reduce((sum, u) => sum + u.loginCount, 0);
  ...
  const handleDeleteJob = async (jobId: number) => {
    try {
      const response = await fetch(`http://localhost:9090/api/jobs/${jobId}`, {
        method: 'DELETE',
        headers: {
  ...
  const handleDeleteUser = (userId: number) => {
    ...
    localStorage.setItem('registeredUsers', JSON.stringify(updatedUsers.map(u => ({ firstName: u.firstName, lastName: u.lastName, email: u.email, role: u.role, companyName: u.companyName }))));
  ...
  const stats = [
    ...
    { 
      ...
      subtitle: `${users.filter(u => u.role === 'JOB_SEEKER').length} Job Seekers, ${users.filter(u => u.role === 'EMPLOYER').length} Employers`
    }
  ];
  ...
  const handleEditSave = async () => {
    try {
      if (editFormData.type === 'job') {
        ...
        const response = await fetch(`http://localhost:9090/api/jobs/${editFormData.id}`, {
          method: 'PUT',
          headers: {
                  ...
                  <Box sx={{ '& > div': { mb: 2, display: 'flex', justifyContent: 'space-between' } }}>
                    ...
                    <Box>
                      ...
                      <Typography variant="h6" color="primary.main">
                        {users.filter(u => u.role === 'EMPLOYER' && u.status === 'Active').length}
                      </Typography>
                    </Box>
                    <Box>
          ...
          {editFormData.type === 'job' ? (
            <Box sx={{ '& > div': { mb: 2 } }}>
              ...
              <TextField
                ...
                onChange={(e) => setEditFormData(prev => ({ ...prev, experienceLevel: e.target.value }))}
              />
              <Box display="flex" gap={2}>
                <TextField
                  ...
                  onChange={(e) => setEditFormData(prev => ({ ...prev, salaryMin: parseInt(e.target.value) }))}
                  sx={{ flex: 1 }}
                />
                <TextField
                  label="Max Salary"
                  ...
                  onChange={(e) => setEditFormData(prev => ({ ...prev, salaryMax: parseInt(e.target.value) }))}
                  sx={{ flex: 1 }}
                />
              </Box>
              <TextField
