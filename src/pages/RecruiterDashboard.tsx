...
const RecruiterDashboard: React.FC = () => {
  ...
  const fetchAllApplications = async () => {
    try {
      ...
      // Fallback: Check localStorage for all applications
      if (employerApps.length === 0) {
        console.log('No API applications found, checking localStorage...');
        const allUsers = Object.keys(localStorage).filter(key => key.startsWith('appliedJobs_'));
        
        for (const userKey of allUsers) {
          const userApps = JSON.parse(localStorage.getItem(userKey) || '[]');
          const userEmail = userKey.replace('appliedJobs_', '');
          
          for (const app of userApps) {
            // Check if this application is for one of the recruiter's jobs
            const matchingJob = employerJobs.find(job => job.id == app.job?.id);
            if (matchingJob) {
              employerApps.push({
                ...app,
        ...
        console.log('LocalStorage applications found:', employerApps);
      }
  ...
  const handleStatusUpdate = async (applicationId: number, newStatus: string) => {
    try {
      ...
      try {
        const response = await fetch(`http://localhost:9090/api/applications/${applicationId}/status`, {
          method: 'PUT',
          headers: {
      ...
      // Update localStorage (works as fallback or primary)
      const allUsers = Object.keys(localStorage).filter(key => key.startsWith('appliedJobs_'));
      for (const userKey of allUsers) {
        const userApps = JSON.parse(localStorage.getItem(userKey) || '[]');
        const updatedApps = userApps.map((app: any) => {
          if (app.id === applicationId || (app.job?.id == application.job?.id && app.job?.title === application.jobTitle)) {
